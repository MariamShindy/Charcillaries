...
public static class Fmt2
{
    ...
    /// <returns></returns>
    public static IHtmlContent WhenCulture(string? arabic, string? english, bool isDefaultArabicWhenEnglishIsNull = false)
    {
        var lang = Env.CurrentCultureCode.ToSupportedLang();
    ...
    public static string PluralizeKey(int count, string singular, string plural)
    {
        if (count == 1)
            return singular;
        else if (count > 1)
